...
#### Add the predictions to the test tibble ----
credit_test_with_pred_boost_tree_tbl <- augment(model_fit_boost_tree, credit_test)
credit_test_with_pred_boost_tree_tbl

#### Create a confusion matrix ----
conf_mat_boost_tree <- conf_mat(
    data     = credit_test_with_pred_boost_tree_tbl,
    truth    = default,
    estimate = .pred_class
)
conf_mat_boost_tree
...
#### Calculate the ROC AUC (area under the curve) ----
credit_roc_auc_boost_tree <- credit_test_with_pred_boost_tree_tbl %>% 
    roc_auc(
        truth    = default,
        estimate = .pred_no
    )
